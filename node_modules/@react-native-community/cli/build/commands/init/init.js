...
async function createTemplateUri(options, version) {
  ...
  const useLegacyTemplate = _semver().default.lt(simpleVersion, TEMPLATE_COMMUNITY_REACT_NATIVE_VERSION);
  _cliTools().logger.debug(`[template]: is '${version} (${simpleVersion})' < '${TEMPLATE_COMMUNITY_REACT_NATIVE_VERSION}' = ` + (useLegacyTemplate ? 'yes, look for template in react-native' : 'no, look for template in @react-native-community/template'));
  ...
  _cliTools().logger.debug(`Using the legacy template because '${TEMPLATE_PACKAGE_LEGACY}' still contains a template folder`);
